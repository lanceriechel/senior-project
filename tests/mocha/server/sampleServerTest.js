  ...
  describe("Employees Tab", function(){
    ...
    describe("Timesheet Tab", function(){
      ...
      it("check that empty timsheets are inactive if submitted", function(){
        ...
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).active == 0);
      });
      ...
      it("check that a timsheet is active with an unapproved RevisionArray", function(){
        ...
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).active == 1);
      });
      ...
      it("adding valid row to timesheet successful", function(){
        ...
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).projectEntriesArray[0].projectId == project);
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).projectEntriesArray[0].EntryArray[0].Comment == 'comment');
      });
      ...
      it("editing a projectComments row is successful", function(){
        ...
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).projectEntriesArray[0].next == 'next');
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).projectEntriesArray[0].issues == 'issues');
      });
      ...
      it("removing a row from a timesheet is successful", function(){
        ...
        chai.assert.notOk(TimeSheet.findOne({'startDate':startDate,'userId':userId}).projectEntriesArray.length == 1);
      });
      ...
      it("submitting an timesheet with invalid parameters fails", function(){
        ...
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).submitted == false);
      });
      ...
      it("submitting a timesheet with valid parameters succeeds", function(){
        ...
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).submitted == true);
      });
      ...
      it("test changing revision of timesheet", function(){
        ...
        var id = TimeSheet.findOne({'startDate':startDate,'userId':userId})._id;
        Meteor.call('updateRevision', id, revision);
        
        chai.assert.ok(TimeSheet.findOne({'startDate':startDate,'userId':userId}).revision[0].totalHours == 6);
      });
